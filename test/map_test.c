...
int test_u8_insert_removal(){
	...
	TEST_ASSERT(cdada_map_first(map, &key, &tmp) == CDADA_E_EMPTY);
	TEST_ASSERT(cdada_map_last(map, &key, &tmp) == CDADA_E_EMPTY);
	...
	char buffer[1024];
	uint32_t used;
	rv = cdada_map_dump(map, 1024, buffer, &used);
	TEST_ASSERT(rv == CDADA_SUCCESS);
	fprintf(stdout, "%s\n", buffer);
...
int _test_u552_insert_removal_traverse(){
	...
	char buffer[2048*8];
	uint32_t used;
	rv = cdada_map_dump(map, 2048*8, buffer, &used);
	TEST_ASSERT(rv == CDADA_SUCCESS);
	fprintf(stdout, "%s\n", buffer);
...
int test_u3552_insert_removal_traverse_custom(){
	...
	uint32_t used;
	rv = cdada_map_dump(map, 2048*8*2, buffer, &used);
	TEST_ASSERT(rv == CDADA_SUCCESS);
	fprintf(stdout, "%s\n", buffer);
